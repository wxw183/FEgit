...
double em=200000000000.0/(1-0.3*0.3),pr=0.3/(1.0-0.3),th=1.0;//EM－杨氏模量， PR－泊松比， TH－板的厚度
...
void str(){
    ...
    f=fprintf(fpstress,"%lg,%lg,%lg,%lg,%lg\n",(x[0]+x[1]+x[2])/3,(y[0]+y[1]+y[2])/3,stress[0],stress[1],(stress[0]+stress[1])*0.5+sqrt((0.5*(stress[1]-stress[0]))*(0.5*(stress[1]-stress[0]))+(stress[2])*(stress[2])));
    fclose(fpstress);
    printf("%d\t",f);
    if(f==-1)exit(0);
}